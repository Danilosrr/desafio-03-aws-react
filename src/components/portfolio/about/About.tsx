...
export default function About({ gitUser }: Readonly<Props>) {
  const { uid } = useParams();
  ...
  const linkedinClick = () => {
    if (editable) {
      setModal(true);
    } else if (uid && getUserData(uid)["linkedin"]) {
    ...
    <>
        ...
        <article className="aboutUser">
          <figure>
            <div className="portrait">
              {gitUser?.avatar_url && (
                <img src={gitUser.avatar_url} alt="user" />
              )}
            </div>
        ...
        <aside className="aboutPitch">
          ...
          <span>
            {gitUser?.login && (
              <button
                onClick={() =>
                  openInNewTab(`https://github.com/${gitUser.login}`)
                }
              >
                Github
              </button>
            )}
            {(storedUser?.linkedin || editable) && (
          ...
          <textarea
            ...
            placeholder={
              editable
                ...
                : "Não há nenhuma história pra contar!"
            }
      ...
      {modal && (
        <Modal
          uid={uid}
          setState={setModal}
          keys={[{ name: "linkedin", required: false }]}
        />
      )}
    </>
  );
